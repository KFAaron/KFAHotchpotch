...
@interface KFABottomMenuView ()

@property (nonatomic,strong) KFAReadProgressView *progressView;
@property (nonatomic,strong) KFAThemeView *themeView;
...
@implementation KFABottomMenuView
...
-(void)dealloc {
    ...
    [[KFAReadConfig shareInstance] removeObserver:self forKeyPath:@"fontSize"];
}
...
- (void)setup {
    ...
    [[KFAReadConfig shareInstance] addObserver:self forKeyPath:@"fontSize" options:NSKeyValueObservingOptionNew context:NULL];
}

- (UIButton *)catalog {
    if (!_catalog) {
        _catalog = [KFAReadUtilites commonButtonSEL:@selector(showCatalog) target:self];
        [_catalog setImage:[UIImage imageNamed:@"reader_cover"] forState:UIControlStateNormal];
    }
    return _catalog;
}

-(KFAReadProgressView *)progressView {
    if (!_progressView) {
        _progressView = [[KFAReadProgressView alloc] init];
...
- (UISlider *)slider {
    if (!_slider) {
        ...
        _slider.minimumTrackTintColor = KFARGB(227, 0, 0);
...
- (UIButton *)nextChapter {
    if (!_nextChapter) {
        _nextChapter = [KFAReadUtilites commonButtonSEL:@selector(jumpChapter:) target:self];
...
- (UIButton *)lastChapter {
    if (!_lastChapter) {
        _lastChapter = [KFAReadUtilites commonButtonSEL:@selector(jumpChapter:) target:self];
...
- (UIButton *)increaseFont {
    if (!_increaseFont) {
        _increaseFont = [KFAReadUtilites commonButtonSEL:@selector(changeFont:) target:self];
...
- (UIButton *)decreaseFont {
    if (!_decreaseFont) {
        _decreaseFont = [KFAReadUtilites commonButtonSEL:@selector(changeFont:) target:self];
...
- (UILabel *)fontLabel {
    if (!_fontLabel) {
        ...
        _fontLabel.text = [NSString stringWithFormat:@"%d",(int)[KFAReadConfig shareInstance].fontSize];
    }
    return _fontLabel;
}

- (KFAThemeView *)themeView {
    if (!_themeView) {
        _themeView = [[KFAThemeView alloc] init];
    }
    return _themeView;
}
...
- (void)changeFont:(UIButton *)sender {
    ...
    if (sender == _increaseFont) {
        ...
        if (floor([KFAReadConfig shareInstance].fontSize) == floor(kMaxFontSize)) {
            return;
        }
        [KFAReadConfig shareInstance].fontSize++;
    } else {
        if (floor([KFAReadConfig shareInstance].fontSize) == floor(kMinFontSize)){
            return;
        }
        [KFAReadConfig shareInstance].fontSize--;
    }
...
- (UIView *)theme2 {
    if (!_theme2) {
        ...
        _theme2.backgroundColor = KFARGB(188, 178, 190);
...
- (UIView *)theme3 {
    if (!_theme3) {
        ...
        _theme3.backgroundColor = KFARGB(190, 182, 162);
...
- (void)changeTheme:(UITapGestureRecognizer *)tap {
    [[NSNotificationCenter defaultCenter] postNotificationName:KFAThemeNotification object:tap.view.backgroundColor];
}
...
@interface KFAReadProgressView ()

@property (nonatomic,strong) UILabel *label;

@end

@implementation KFAReadProgressView
...
- (UILabel *)label {
    if (!_label) {
        ...
        _label.font = [UIFont systemFontOfSize:[KFAReadConfig shareInstance].fontSize];
