...
@interface KFAReadView ()

@property (nonatomic,strong) KFAMagnifierView *magnifierView;

@end

@implementation KFAReadView
{
    NSRange _selectRange;
    NSRange _calRange;
    NSArray *_pathArray;
    
    UIPanGestureRecognizer *_pan; // 滑动手势
    //滑动手势有效区间
    CGRect _leftRect;
    CGRect _rightRect;
...
- (void)showMagnifier {
    if (!_magnifierView) {
        self.magnifierView = [[KFAMagnifierView alloc] init];
...
- (void)longPress:(UILongPressGestureRecognizer *)longPress {
    ...
    if (longPress.state == UIGestureRecognizerStateBegan || longPress.state == UIGestureRecognizerStateChanged) {
        CGRect rect = [KFAReadParser parserRectWithPoint:point range:&_selectRange frameRef:_frameRef];
        [self showMagnifier];
...
- (void)pan:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateBegan || pan.state == UIGestureRecognizerStateChanged) {
        ...
        if (_selectState) {
            NSArray *path = [KFAReadParser parserRectsWithPoint:point range:&_selectRange frameRef:_frameRef paths:_pathArray direction:_direction];
            _pathArray = path;
            [self setNeedsDisplay];
        }
    }
...
- (void)menuCopy:(id)sender {
    ...
    [KFAReadUtilites showAlertTitle:@"成功复制以下内容" content:pasteboard.string];
}
...
- (void)menuNote:(id)sender {
    ...
    UIAlertAction *confirm = [UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        KFANoteModel *model = [[KFANoteModel alloc] init];
        model.content = [_content substringWithRange:_selectRange];
        model.note = alertController.textFields.firstObject.text;
        model.date = [NSDate date];
        [[NSNotificationCenter defaultCenter] postNotificationName:KFANoteNotification object:model];
    }];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_imageArray.count) {
        for (KFAImageData * imageData in self.imageArray) {
...
- (void)fillImagePosition:(KFAImageData *)imageData {
