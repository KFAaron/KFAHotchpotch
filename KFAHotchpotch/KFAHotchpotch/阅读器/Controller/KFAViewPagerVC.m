...
@interface KFAViewPagerVC () <UIPageViewControllerDelegate,UIPageViewControllerDataSource>
{
    ...
    KFAViewPagerTitleButton *oldButton;
...
-(void)setDataSource:(id<KFAViewPagerVCDataSource>)dataSource {
...
- (void)reload {
    if ([self.dataSource respondsToSelector:@selector(numberOfViewControllersInViewPager:)]) {
        ...
        for (int i = 0; i<numberOfViewController; i++) {
            ...
            if ([self.dataSource respondsToSelector:@selector(viewPager:titleWithIndexOfViewControllers:)]) {
                ...
                if ([self.dataSource respondsToSelector:@selector(viewPager:titleButtonStyle:)]) {
                    ...
                    button = [[KFAViewPagerTitleButton alloc] init];
                }
        ...
        if (mutableArrayOfBtn.count && ((UIButton *)mutableArrayOfBtn.lastObject).frame.origin.x + ((UIButton *)mutableArrayOfBtn.lastObject).frame.size.width<self.view.frame.size.width) //当所有按钮尺寸小于屏幕宽度的时候要重新布局
        {
            ...
            for (KFAViewPagerTitleButton *button in mutableArrayOfBtn) {
...
- (void)p_titleButtonClick:(KFAViewPagerTitleButton *)sender {
...
- (void)p_titleButtonConvert:(KFAViewPagerTitleButton *)sender {
    oldButton.selected = NO;
    sender.selected = YES;
    oldButton = sender;
}
...
@implementation KFAViewPagerTitleButton
