...
@interface KFAChapterModel ()

@property (nonatomic,strong) NSMutableArray *pageArray;

@end

@implementation KFAChapterModel
...
- (id)copyWithZone:(NSZone *)zone {
    KFAChapterModel *model = [[KFAChapterModel allocWithZone:zone] init];
...
+ (KFAChapterModel *)chapterWithEpub:(NSString *)chapterpath title:(NSString *)title imagePath:(NSString *)path {
    ...
    KFAChapterModel *model = [[KFAChapterModel alloc] init];
    model.title = title;
    model.epubImagePath = path;
    model.type = KFAReaderTypeEpub;
    ...
    [model paginateEpubWithBounds:CGRectMake(0,0, kScreenWidth-kLeftSpacing-kRightSpacing, kScreenHeight-kTopSpacing-kBottomSpacing)];
    return model;
}
...
- (void)updateFont {
    if (_type==KFAReaderTypeEpub) {
        [self paginateEpubWithBounds:CGRectMake(0,0, kScreenWidth-kLeftSpacing-kRightSpacing, kScreenHeight-kTopSpacing-kBottomSpacing)];
    } else {
        [self paginateWithBounds:CGRectMake(kLeftSpacing, kTopSpacing,kScreenWidth-kLeftSpacing-kRightSpacing, kScreenHeight-kTopSpacing-kBottomSpacing)];
    }
}
...
@implementation KFAChapterModel (KFAParseData)
...
-(void)parserEpubToDictionary {
    ...
    while (![scanner isAtEnd]) {
        if ([scanner scanString:@"<img>" intoString:NULL]) {
            ...
            CGFloat width = kScreenWidth - kLeftSpacing - kRightSpacing;
            CGFloat height = kScreenHeight - kTopSpacing - kBottomSpacing;
            ...
            //存储图片信息
            KFAImageData *imageData = [[KFAImageData alloc] init];
...
-(void)paginateEpubWithBounds:(CGRect)bounds {
    ...
    for (NSDictionary *dic in _epubContent) {
        if ([dic[@"type"] isEqualToString:@"txt"]) {
            //解析文本
            NSLog(@"--%.2f",[KFAReadConfig shareInstance].fontSize);
            NSDictionary *attr = [KFAReadParser parserAttribute:[KFAReadConfig shareInstance]];
            ...
            //解析图片
            NSAttributedString *subString = [KFAReadParser parserEpubImageWithSize:dic config:[KFAReadConfig shareInstance]];
            [attrString appendAttributedString:subString];
...
- (void)paginateWithBounds:(CGRect)bounds {
    ...
    NSDictionary *attribute = [KFAReadParser parserAttribute:[KFAReadConfig shareInstance]];
