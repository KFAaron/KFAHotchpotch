...
#import "KFAPlayerLyricsTableview.h"
#import "KFAPlayerLyricsTableViewCell.h"
#import "KFAMusicPlayer.h"

static NSString *KFAPlayerLyricStateKey = @"state";
static NSString *KFAPlayerLyricCurrentTimeKey = @"currentTime";
static NSString *KFAPlayerLyricCurrentAudioInfoModelKey = @"currentAudioInfoModel";
static NSString *KFAPlayerLyricConstMark = @"####";

static NSString *KFAPlayerlyricNoticeStr_zh_unavailable = @"暂无可用歌词";
static NSString *KFAPlayerlyricNoticeStr_en_unavailable = @"Unavailable Data";

#define KFA_FONTSIZE(size) ((size)/1334.0)*kScreenHeight

@interface KFAPlayerLyricsTableview () <UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate>
...
@implementation KFAPlayerLyricsTableview

- (void)dealloc{
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    [[KFAMusicPlayer shareInstance] removeObserver:self forKeyPath:KFAPlayerLyricStateKey];
    [[KFAMusicPlayer shareInstance] removeObserver:self forKeyPath:KFAPlayerLyricCurrentTimeKey];
    [[KFAMusicPlayer shareInstance] removeObserver:self forKeyPath:KFAPlayerLyricCurrentAudioInfoModelKey];
}
...
- (instancetype)init{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(kfa_progressSliderDragEnd) name:KFAMusicPlayerNotificationProgressSliderDragEnd object:nil];
        [[KFAMusicPlayer shareInstance] addObserver:self forKeyPath:KFAPlayerLyricStateKey options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionInitial context:nil];
        [[KFAMusicPlayer shareInstance] addObserver:self forKeyPath:KFAPlayerLyricCurrentTimeKey options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionInitial context:nil];
        [[KFAMusicPlayer shareInstance] addObserver:self forKeyPath:KFAPlayerLyricCurrentAudioInfoModelKey options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionInitial context:nil];
    }
    return self;
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    if (object == [KFAMusicPlayer shareInstance]) {
        if ([keyPath isEqualToString:KFAPlayerLyricStateKey]) {
            if ([KFAMusicPlayer shareInstance].state == KFAMusicPlayerStatePlaying) {
                [self kfa_resumeLayer:self.maskLayer];
            }else{
                [self kfa_pauseLayer:self.maskLayer];
            }
        }else if ([keyPath isEqualToString:KFAPlayerLyricCurrentTimeKey]){
            [self kfa_updateLyricsTextWithAnimation:YES];
            
        }else if ([keyPath isEqualToString:KFAPlayerLyricCurrentAudioInfoModelKey]){
            [self kfa_lyricAnalyze];
        }
    }
}
#pragma mark - 更新歌词信息
- (void)kfa_updateLyricsTextWithAnimation:(BOOL)animation{
    //停止更新
    if (self.isStopUpdateLrc) {[self kfa_pauseLayer:self.maskLayer];return;}
    ...
    //获取当前行
    CGFloat currentTime = [KFAMusicPlayer shareInstance].currentTime;
    ...
    //当前行移动到中间
    if (self.isProgressSliderDragEnd || !animation) {
        [self kfa_scrollToMiddleCellAnimation:NO];
    }else{
        [self kfa_scrollToMiddleCellAnimation:YES];
    }
...
- (void)updateCurrentCellUIWithAnimation:(BOOL)animation{
    ...
    if (self.currentIndexArray.count > 1) {
        for (int i = 0; i < self.currentIndexArray.count-1; i++) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                KFAPlayerLyricsTableViewCell *cell = (KFAPlayerLyricsTableViewCell *)[self cellForRowAtIndexPath:indexPath];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        //当前行
        KFAPlayerLyricsTableViewCell *cell = (KFAPlayerLyricsTableViewCell *)[self cellForRowAtIndexPath:self.currentIndexPath];
        if (!self.isProgressSliderDragEnd) {
            //如果当前行无歌词，记录位置并返回
            NSString *noNilStr = [self removeNilWithStr:cell.foregroundLrcLabel.text];
            if (noNilStr.length == 0 || [noNilStr isEqualToString:KFAPlayerLyricConstMark]) {
        ...
        if (self.currentLineLrcForegroundTextColor) {
            ...
            if (self.currentIndex < self.timeArray.count - 1) {
                ...
                if (![self IsNilWithStr:self.lyricArray.lastObject]) {//如果最后一句不为空
                    duration = fabs([KFAMusicPlayer shareInstance].totalTime - [self.timeArray[self.currentIndex] floatValue]-0.2);
                }
            }
...
/**cell移动*/
- (void)kfa_scrollToMiddleCellAnimation:(BOOL)aniamtion{
...
/**暂停恢复*/
-(void)kfa_pauseLayer:(CALayer*)layer{
    CFTimeInterval pausedTime = [layer convertTime:CACurrentMediaTime() fromLayer:nil];
    layer.speed = 0.0;
    layer.timeOffset = pausedTime;
}
-(void)kfa_resumeLayer:(CALayer*)layer{
...
- (void)kfa_lyricAnalyze{
    ...
    if ([self.audioUrl.absoluteString isEqualToString:url.absoluteString]) {
        ...
        NSString *lyric = [KFAMusicPlayer shareInstance].currentAudioModel.audioLyric;
...
- (void)lyricAnalyze:(NSString *)lyric{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        //得到数据调用一次更新信息
        [self kfa_updateLyricsTextWithAnimation:NO];
    });
}
/**位置复原*/
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    KFAPlayerLyricsTableViewCell *cell = (KFAPlayerLyricsTableViewCell *)[self tableView:tableView cellForRowAtIndexPath:indexPath];
    return cell.frame.size.height;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    NSString *CellIdentifier = [NSString stringWithFormat:@"Cell%ld%ld", (long)indexPath.section, (long)indexPath.row];
    KFAPlayerLyricsTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil) {
        cell = [[KFAPlayerLyricsTableViewCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:CellIdentifier];
    ...
    if (indexPath.row < self.lyricArray.count) {//安全性判断
        ...
        if (noNilLrc.length == 0) {
            height = 0;
        }else if([noNilLrc isEqualToString:KFAPlayerLyricConstMark]){
            lrc = @"";
        }
...
- (void)kfa_progressSliderDragEnd{
...
- (UILabel *)noticeLabel{
    if (!_noticeLabel) {
        ...
        if ([self isZhHansApplesLauguages]) {
            _noticeLabel.text = KFAPlayerlyricNoticeStr_zh_unavailable;
        }else{
            _noticeLabel.text = KFAPlayerlyricNoticeStr_en_unavailable;
        }
