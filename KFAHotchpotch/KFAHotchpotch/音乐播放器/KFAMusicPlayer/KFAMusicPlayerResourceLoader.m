//
//  KFAMusicPlayerResourceLoader.m
//  KFAHotchpotch
//
...
@interface KFAMusicPlayerResourceLoader ()

@property (nonatomic, strong) NSMutableArray<AVAssetResourceLoadingRequest *> *requestList;
@property (nonatomic, strong) KFAMusicPlayerRequestManager *requestManager;

@end

@implementation KFAMusicPlayerResourceLoader
...
- (void)newTaskWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest cache:(BOOL)cache {
    ...
    self.requestManager = [[KFAMusicPlayerRequestManager alloc] initWithUrl:loadingRequest.request.URL];
    if (fileLength > 0) {
...
- (BOOL)finishLoadingWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    [loadingRequest.dataRequest respondWithData:[KFAMusicCacheManager readDataFromTempFileWith:requestedOffset length:respondLength]];
